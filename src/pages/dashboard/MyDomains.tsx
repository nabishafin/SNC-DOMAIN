...
import { Search, Settings as SettingsIcon, RefreshCw, MoreHorizontal, ArrowUpRight } from 'lucide-react';
...
const MyDomains = () => {
    ...
    const [filterStatus, setFilterStatus] = useState<'all' | 'active' | 'expiring-soon'>('all');
        ...
        <DashboardLayout>
            ...
            <div className="flex flex-col md:flex-row gap-4 mb-6">
                ...
                <div className="flex items-center gap-2 p-1 bg-white rounded-xl border border-neutral-200 shadow-sm overflow-x-auto">
                    {(['all', 'active', 'expiring-soon'] as const).map((status) => (
                        <button
                            key={status}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {filteredDomains.map((domain) => (
                    <Card key={domain.id} className="border-none shadow-soft-md hover:shadow-soft-lg transition-all duration-300 group">
                        <CardBody className="p-6">
                            ...
                            <div className="flex gap-3">
                                <Link to={`/dashboard/domains/${domain.id}`} className="flex-1">
                                    <Button variant="primary" size="md" className="w-full shadow-md shadow-primary-500/10">
                                        <SettingsIcon className="w-4 h-4 mr-2" />
                                        Manage
                                    </Button>
                                </Link>
                                <Button variant="outline" size="md" className="flex-1 bg-white">
                                    <RefreshCw className="w-4 h-4 mr-2" />
                                    Renew
                                </Button>
                                <Button variant="ghost" size="md" className="px-2 text-neutral-400 hover:text-neutral-600">
