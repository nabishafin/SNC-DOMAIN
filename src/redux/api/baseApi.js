...
const BASE_URL = import.meta.env.VITE_BASE_URL || "https://virtual-cfr-then-matrix.trycloudflare.com/api";
...
const baseQueryWithReauth = async (args, api, extraOptions) => {
  // Skip re-auth logic for login and register endpoints
  ...
  if (result?.error && result.error.status === 401) {
    ...
    if (refreshToken) {
      try {
        ...
        window.location.href = "/login"; // Fixed: Redirect to correct login path
...
export const baseApi = createApi({
  ...
  tagTypes: ["auth", "dashboardStats", "admin-users", "earnings-overview", "Faqs", "payments", "domains", "dns", "ssl", "contacts", "dnssec", "dnssec-keys", "glueRecords", "zones", "zone-records", "ns-sets", "dyndns", "dyndns-subscription", "dyndns-products"],
  endpoints: () => ({}),
